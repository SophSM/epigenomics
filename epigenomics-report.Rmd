...
bedGraphToBigWig H3K4me3-sorted2.bedGraph GRCh38_EBV.chrom.sizes.tsv H3K4me3.bw
...
# bedGraphToBigWig input-sorted2.bedGraph GRCh38_EBV.chrom.sizes.tsv input.bw
```
...
Computing matrix with `deepTools`, using the `reference-point` sub-command since it refers to a position within a BED region and can plot us regions upstream and downstream of said position. In this case, we are using the promoters and the enhancers as reference positions, so we run this comand for each of these bed files.

```{bash eval = F}
module load deeptools/2.5.3
computeMatrix reference-point -S H3K4me3.bw -R promoters38.bed --referencePoint center -a 2000 -b 2000 -o promoter-mat.tab.gz
computeMatrix reference-point -S H3K4me3.bw -R enhancers38.bed --referencePoint center -a 2000 -b 2000 -o enhancer-mat.tab.gz
```

We also plot our matrix in a Heatmap to visualize possible enrichment

```{bash eval = F}
plotHeatmap -m promoter-mat.tab.gz -o promoter-heatmap.png
plotHeatmap -m enhancer-mat.tab.gz -o enhancer-heatmap.png
```
